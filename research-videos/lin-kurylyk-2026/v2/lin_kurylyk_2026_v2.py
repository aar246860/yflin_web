...
# noqa: SIZE_OK
# One-file ownership keeps the exported 18-scene animation reproducible without private runtime modules.
...
class FieldFitEvidence:
    ...
    amplitude_observations: tuple[tuple[float, float, float], ...]
    phase_observations: tuple[tuple[float, float, float], ...]
    amplitude_fit: tuple[tuple[float, float], ...]
    phase_fit: tuple[tuple[float, float], ...]
    source_locator: str
...
FIELD_FIT_EVIDENCE: Final = (
    FieldFitEvidence(
        name="Tuolumne",
        diffusivity_tex=r"2.2\times10^4",
        tau_q=2.28,
        tau_h=0.12,
        x_max=110.0,
        amplitude_range=(0.0, 1.08, 0.25),
        phase_range=(0.0, 8.0, 2.0),
        amplitude_observations=((17.0, 0.92, 0.07), (52.0, 0.56, 0.12), (100.0, 0.43, 0.12)),
        phase_observations=((17.0, 1.45, 0.30), (52.0, 1.75, 0.50), (100.0, 3.20, 1.45)),
        amplitude_fit=((0.0, 1.00), (17.0, 0.84), (52.0, 0.60), (100.0, 0.36), (110.0, 0.32)),
        phase_fit=((0.0, 0.00), (17.0, 0.90), (52.0, 2.45), (100.0, 4.90), (110.0, 5.40)),
        source_locator="Figure 2 and Table 1",
    ),
    FieldFitEvidence(
        name="Meghna",
        diffusivity_tex=r"8.7\times10^3",
        tau_q=2.07,
        tau_h=38.60,
        x_max=92.0,
        amplitude_range=(0.4, 1.08, 0.2),
        phase_range=(0.0, 0.10, 0.025),
        amplitude_observations=((20.0, 0.90, 0.025), (75.0, 0.89, 0.05), (80.0, 0.90, 0.03)),
        phase_observations=((20.0, 0.047, 0.008), (75.0, 0.052, 0.020), (80.0, 0.034, 0.020)),
        amplitude_fit=((0.0, 0.98), (20.0, 0.94), (75.0, 0.85), (80.0, 0.84), (90.0, 0.81)),
        phase_fit=((0.0, 0.005), (20.0, 0.012), (75.0, 0.033), (80.0, 0.035), (90.0, 0.040)),
        source_locator="Figure 3 and Table 1",
    ),
)
...
class LinKurylyk2026V2(Scene):
    ...
    def scene_10_m07_field_fit(self) -> None:
        panels = VGroup()
        for site in FIELD_FIT_EVIDENCE:
            amplitude_axes = Axes(
                x_range=[0, site.x_max, 25],
                y_range=list(site.amplitude_range),
                x_length=4.45,
                y_length=1.55,
            ...
            amp, phi = published_field_curves(
                amplitude_axes,
                phase_axes,
                amplitude_points=site.amplitude_fit,
                phase_points=site.phase_fit,
            )
            observations = VGroup()
            for x_value, y_value, error in site.amplitude_observations:
            ...
            for x_value, y_value, error in site.phase_observations:
                point = phase_axes.c2p(x_value, y_value)
                observations.add(
                    Line(
                        phase_axes.c2p(x_value, max(site.phase_range[0], y_value - error)),
                        phase_axes.c2p(x_value, min(site.phase_range[1], y_value + error)),
                        color=INK,
                        stroke_width=2,
                    ),
                    Dot(point, radius=0.06, color=INK),
                )
            site_label = text(site.name, 23).move_to([0, 2.25, 0])
            parameter_label = MathTex(
                rf"D={site.diffusivity_tex}\ \mathrm{{m^2/d}},\ \tau_q={site.tau_q:.2f}\,d,\ \tau_h={site.tau_h:.2f}\,d",
                color=INK,

...
def _require_scene_coverage(scene: Scene, pending_items: Sequence[Mobject], frame_items: Sequence[Mobject]) -> None:
...
def assert_no_overlap(labels: Sequence[Mobject], blockers: Sequence[Mobject], min_gap: float) -> None:
        ...
        for blocker in _overlap_leaves(blockers):
            if bbox_gap(label, blocker) < min_gap:
                raise LayoutContractError(
                    f"layout overlap risk: label {label.__class__.__name__} is too close to {blocker.__class__.__name__}"
                )


def assert_mutually_clear(items: Sequence[Mobject], min_gap: float) -> None:
...
def assert_within_frame(
    ...
    assert_no_unintended_overlap(items, intentional_overlaps)
    ...
    for item in _overlap_leaves(items):
        ...
        if outside:
            raise LayoutContractError(f"layout frame risk: {item.__class__.__name__} extends outside the safe frame")
...
def assert_inside(container: Mobject, items: Sequence[Mobject], min_gap: float = 0.0) -> None:
    ...
    for item in items:
        ...
        if outside or not shape_contains(container, item, min_gap):
            raise LayoutContractError(f"layout boundary risk: {item.__class__.__name__} extends outside {container.__class__.__name__}")
...
def assert_scene_layout(
    *,
    ...
    assert_mutually_clear(labels, min_gap)
    assert_no_overlap(labels, blockers, min_gap)
    assert_within_frame(frame_items, frame_margin, intentional_overlaps)
